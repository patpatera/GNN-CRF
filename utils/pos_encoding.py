...
class CentralityPE(nn.Module):

    def __init__(self, num_in_degree, num_out_degree, hidden_dim):
        super().__init__()

        self.num_in = num_in_degree
        self.num_out = num_out_degree
        self.in_degree_encoder = nn.Embedding(num_in_degree, hidden_dim, padding_idx=0)
        self.out_degree_encoder = nn.Embedding(num_out_degree, hidden_dim, padding_idx=0)

    def forward(self, data):
        num_nodes = data.x.shape[0]
        in_degree = degree(data.edge_index[1], num_nodes=num_nodes).long()
        out_degree = degree(data.edge_index[0], num_nodes=num_nodes).long()
        
        data.x = data.x + self.in_degree_encoder(in_degree) + self.out_degree_encoder(out_degree)
