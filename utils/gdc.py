...
from torch_geometric.utils import add_self_loops, is_undirected, to_dense_adj, degree
...
class GDC(BaseTransform):
    ...
    def __call__(self, data: Data) -> Data:
        ...
        if self.exact:
            ...
            diff_mat = self.diffusion_matrix_exact(edge_index, edge_weight, N, None,
    ...
    def transition_matrix(
        ...
        """
        W = torch.ones(edge_index.size(1), device=edge_index.device)

        if normalization == 'sym':
            row, col = edge_index
            #deg = scatter_add(W, row, dim=0, dim_size=num_nodes)
        ...
        **kwargs,
        ...
        if method == 'ppr':
            ...
            edge_index, edge_weight = add_self_loops(edge_index, edge_weight,
                                                     fill_value=1,
                                                     num_nodes=num_nodes)
            mat = to_dense_adj(edge_index, edge_attr=edge_weight).squeeze() - (kwargs['alpha'] - 1)
            diff_matrix = kwargs['alpha'] * torch.inverse(mat)
            print(f"Diff: {(diff_matrix<0).sum().item()}")
        ...
        Args:
            ...
            method (str): Diffusion method:
                ...
                1. :obj:`"ppr"`: Use personalized PageRank as diffusion.
                   ...
                   - **alpha** (*float*) - Return I Ii in PPR.
                     Commonly lies in :obj:`[0.05, 0.2]`.
